...
import { fetchSensorData, SensorData, isDataFromCache, getCacheTimestamp, debugCachedData } from '../services/api';
...
const Analytics: React.FC = () => {
  ...
  useEffect(() => {
    loadData();
    const interval = setInterval(loadData, 120000); // 2 dakika
          ...
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <button 
                ...
                onClick={() => {
                  console.log('🔍 ANALYTICS DEBUG: Veri durumu kontrol ediliyor...');
                  ...
                  console.log('📊 İlk 3 veri:', sensorData.slice(0, 3));
                  debugCachedData();
                  loadData();
                }}
        ...
        <Card className="app-card mt-4">
          <Card.Body>
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h6 className="mb-0">📋 Ham Veri Tablosu</h6>
              <small className="text-muted">Toplam {sensorData.length} kayıt</small>
            </div>
            <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
              <Table striped hover responsive>
                ...
                <tbody>
                  {sensorData
                    .sort((a, b) => new Date(b.measurement_time || b.measurementTime).getTime() - new Date(a.measurement_time || a.measurementTime).getTime())
                    .slice(0, 50)
                    .map((data, index) => (
                      <tr key={data.id || index}>
                        <td>{index + 1}</td>
                        <td>{new Date(data.measurement_time || data.measurementTime).toLocaleString('tr-TR')}</td>
                        <td className="fw-bold">{data.temperature?.toFixed(2)}</td>
                        <td className="fw-bold">{data.humidity?.toFixed(2)}</td>
                        <td>{data.sensorId || 'N/A'}</td>
                        <td>
                          <Badge bg={data.temperature > 25 || data.humidity > 70 ? 'danger' : 'success'}>
                            {data.temperature > 25 || data.humidity > 70 ? 'Kritik' : 'Normal'}
                          </Badge>
                        </td>
                      </tr>
                    ))}
